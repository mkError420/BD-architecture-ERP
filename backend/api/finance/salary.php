...
switch ($method) {
    case 'GET':
        ...
        $stmt = $db->prepare(
            "SELECT sp.*, e.name as employee_name, e.employee_code, e.role as employee_role
             ...
             $where ORDER BY sp.created_at DESC LIMIT $perPage OFFSET $offset"
        );
        $stmt->execute($params);
    ...
    case 'POST':
        ...
        $month = $body['payment_month'] ?? '';
        if (!$empId || !$month) sendError('Employee and month required');
        ...
        $db->prepare(
            ...
            $body['status'] ?? 'pending', $user['id'], $body['notes'] ?? null
        ]);
    ...
    case 'PATCH':
        ...
        $body = getJsonBody();
        $fields = []; $params = [];
        $allowed = ['basic_salary','overtime_pay','bonus','deduction','net_salary','payment_date',
